v5.14.9: 長文お知らせ表示と管理OVERVIEW安定化。
## v5.14.5
管理アプリのJavaScript構文エラー（ブラウザ組み込みの `top` と関数名が衝突）を修正。
...
## v5.14.6
- お知らせをタップして既読にするとホーム上のお知らせカードを閉じます。次の新しいお知らせは再び表示されます。
- ホーム画面追加済み（standalone起動を確認済み / appinstalled済み）の端末では、右上プロフィールの赤丸を消します。
- Service Workerキャッシュをv5.14.6へ更新。


## v5.14.7
- 既読のお知らせを『お知らせ』一覧から見直せるようにしました。
- 未読のお知らせだけホームにNEW表示し、読むとホームから消えます。
- 初回ログイン完了時に必ずページ最上部へ戻るようにしました。


## v5.14.8
- ホームのお知らせをタップすると全画面の詳細リーダーで表示
- お知らせ本文を左揃えに統一
- 詳細を閉じたタイミングで既読化（一覧からは再閲覧可能）
- Supabase Realtime購読 + 12秒ポーリングのフォールバックで新着を自動反映
- 初回ログイン直後に必ずページ最上部へ戻る処理を強化


## v5.15.0
- 新着の思い出に1人1回の「いいね」を追加（解除可能）
- 管理アプリに人気の思い出ランキングを追加
- KENJI MODEの到着状況をSupabaseへ同期し、一般アプリへ反映
- KENJI MODEのマップボタンを実動化
- 写真・動画・ひとこと導線を一般投稿画面へ接続
- デモリセットの保存キー不一致を修正
- `supabase-setup-v5150.sql` を1回実行してください


## v5.15.2
- 新着の思い出で「いいね」を押しても横スクロール位置が先頭へ戻らないよう修正
- Realtimeでいいね数を再取得した場合も現在位置を維持
- 投稿カテゴリに「その他」を追加
- Service Worker cache: homes20-v5152

## v5.15.2
- KENJI RIDE MODE 2.0: GPS速度・走行距離・経過時間・目的地までの直線距離を同時表示。
- 地図を走行画面に常時表示し、現在地自動追従・12秒後の追従復帰・現在地へ戻るを追加。
- 目的地を走行中に変更可能。Google Maps自転車ナビへの導線を追加。
- 一時停止・終了を追加。
